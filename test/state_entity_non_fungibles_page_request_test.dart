import 'package:test/test.dart';
import 'package:unofficial_babylon_gateway_api/unofficial_babylon_gateway_api.dart';

// tests for StateEntityNonFungiblesPageRequest
void main() {
  final instance = StateEntityNonFungiblesPageRequestBuilder();
  // TODO add properties to the builder and call build()

  group(StateEntityNonFungiblesPageRequest, () {
    // LedgerStateSelector atLedgerState
    test('to test the property `atLedgerState`', () async {
      // TODO
    });

    // This cursor allows forward pagination, by providing the cursor from the previous request.
    // String cursor
    test('to test the property `cursor`', () async {
      // TODO
    });

    // The page size requested.
    // int limitPerPage
    test('to test the property `limitPerPage`', () async {
      // TODO
    });

    // Bech32m-encoded human readable version of the address.
    // String address
    test('to test the property `address`', () async {
      // TODO
    });

    // ResourceAggregationLevel aggregationLevel
    test('to test the property `aggregationLevel`', () async {
      // TODO
    });

    // StateEntityNonFungiblesPageRequestOptIns optIns
    test('to test the property `optIns`', () async {
      // TODO
    });

  });
}
