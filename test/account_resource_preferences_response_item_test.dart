import 'package:test/test.dart';
import 'package:unofficial_babylon_gateway_api/unofficial_babylon_gateway_api.dart';

// tests for AccountResourcePreferencesResponseItem
void main() {
  final instance = AccountResourcePreferencesResponseItemBuilder();
  // TODO add properties to the builder and call build()

  group(AccountResourcePreferencesResponseItem, () {
    // Bech32m-encoded human readable version of the address.
    // String resourceAddress
    test('to test the property `resourceAddress`', () async {
      // TODO
    });

    // AccountResourcePreferenceRule resourcePreferenceRule
    test('to test the property `resourcePreferenceRule`', () async {
      // TODO
    });

    // The most recent state version underlying object was modified at.
    // int lastUpdatedAtStateVersion
    test('to test the property `lastUpdatedAtStateVersion`', () async {
      // TODO
    });

  });
}
